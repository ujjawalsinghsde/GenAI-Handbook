...
A **prompt** is the **input message** you send to a Large Language Model (LLM) to get a response.

In simple terms:

> **Prompt = Instructions + Context + Input**

Example (plain English):

```
Explain Kubernetes to a beginner in simple words.
```

The model **does not think** — it only **predicts the next best response based on your prompt**.

That’s why:

* Good prompt → good output
* Poor prompt → confusing or wrong output

---

## 2️⃣ Why Prompt Design is Critical in GenAI

LLMs are:

* Extremely powerful
* Extremely sensitive to wording
...
This is why **Prompt Engineering** exists as a skill and job role.
...
## 3️⃣ Temperature Parameter (Very Important Concept)

Before prompts, you must understand **temperature**.
...
| `0.5`       | Balanced                                  |
| `1.0+`      | Creative, varied, sometimes unpredictable |

### Simple Rule

* **Factual tasks** → Low temperature (0–0.3)
* **Creative tasks** → Higher temperature (0.7–1.5)

📌 This correction is important because many beginners misunderstand this.

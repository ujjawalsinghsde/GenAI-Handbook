...
## 1. **Why Do We Even Need Vector Stores?**

When building a RAG (Retrieval-Augmented Generation) system or any semantic search system, we need:

1. **To convert text into numerical vectors (embeddings).**
2. **To store these vectors with their metadata.**
3. **To quickly search for similar vectors based on the query.**
...
* Similar concepts with different words are ignored.
* Irrelevant results may appear due to keyword overlap.

**Embeddings solve this by representing meaning in a vector space.**
Two texts with similar semantic meaning will have vectors close to each other.

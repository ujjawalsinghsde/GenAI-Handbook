...
Before Runnables, LangChain provided specialized components (LLMs, PromptTemplates, Retrievers, Parsers) but each with different method signatures and execution patterns. This inconsistency created several problems:

---

## Core Idea of Runnable

A Runnable is a standardized wrapper that defines any executable step in an AI workflow. It establishes a common execution contract:
...
Runnables follow a functional composition pattern where:

* Each Runnable performs a single, well-defined operation
* Runnables compose through a consistent pipe operator (`|`)
* Complex workflows are built by chaining simple components
* No special handling or adapter code is required
